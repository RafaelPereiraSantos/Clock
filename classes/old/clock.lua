...
function clock.new.analog_clock(current_hour, current_minute, current_second, pos_x, pos_y)
  ...
  new_clock.draw = function(self, gc)
    local font = gc.getFont()

    gc.setColor(color.WHITE)
    gc.circle("fill", self.pos_x, self.pos_y, self.radio)
    ...
    if self.digital_version then
      ...
      gc.print(digital_time, self.pos_x - (text_width / 2), self.pos_y + self.radio + 10)
    end
    ...
    self.second_pointer:draw(gc)
    self.minute_pointer:draw(gc)
    self.hour_pointer:draw(gc)
  end
